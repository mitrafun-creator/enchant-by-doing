package aiefu.ebd.data.materialoverrides;
...
public class MaterialOverrides {
    public static final MaterialData defaultMatData = new MaterialData(EBDCommon.config.maxEnchantments, EBDCommon.config.maxCurses, EBDCommon.config.enchantmentLimitIncreasePerCurse);
    ...
    public MaterialOverrides(HashMap<String, MaterialData>  toolsMatOverrides, HashMap<String, MaterialData>  armorMatOverrides, HashMap<String, MaterialData>  hardOverrides){
        ...
        for (Item item : BuiltInRegistries.ITEM){
            if(item instanceof TieredItem ti){
                ...
                MaterialData data = armorMatOverrides.get(am.getMaterial().unwrapKey().map(key -> key.location().getPath()).orElse(""));
                if(data != null){
    ...
    public static void generateDefault() throws IOException {
        Path folder = Paths.get("./config/ebd/material-overrides");
        if(!Files.exists(folder)){
            Files.createDirectories(folder);
        }
        Gson gson = EBDCommon.getGson();
        String tools = "./config/ebd/material-overrides/tools.json";
        ...
        String armor = "./config/ebd/material-overrides/armor.json";
        ...
        String items = "./config/ebd/material-overrides/items.json";
    ...
    public static MaterialOverrides read() throws FileNotFoundException {
        ...
        HashMap<String, MaterialData> toolsMat = gson.fromJson(new FileReader("./config/ebd/material-overrides/tools.json"), type);
        HashMap<String, MaterialData>  armorMat = gson.fromJson(new FileReader("./config/ebd/material-overrides/armor.json"), type);
        HashMap<String, MaterialData>  hardOverrides = gson.fromJson(new FileReader("./config/ebd/material-overrides/items.json"), type);
