...
public class ColorDataHolder{
    ...
    public ColorDataHolder(BackgroundColorData bcd, SliderColorData scd, TextSettings tx){
        ...
        this.textActiveColor = TextColor.parseColor(tx.activeColor).result().map(TextColor::getValue).orElse(0xFFFFFF);
        this.textInactiveColor = TextColor.parseColor(tx.inactiveColor).result().map(TextColor::getValue).orElse(0x808080);
        this.dropShadow = tx.dropShadow;

        this.searchBarHintColor = TextColor.parseColor(tx.searchBarHintColor).result().map(TextColor::getValue).orElse(0x808080);
