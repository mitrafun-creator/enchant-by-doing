...
public class EnchantmentListWidget extends AbstractScrollWidget {
    ...
    protected void renderContents(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        boolean mouseInWidget = withinContentAreaPoint(mouseX, mouseY) && !overlayActive;
        if(!mouseInWidget) {
            mouseX = -1;
            mouseY = -1;
        }
        for (EnchButtonWithData b : enchantments) {
            b.myHovered = mouseInWidget
                && mouseX >= b.getX()
                && mouseX < b.getX() + b.getWidth()
                && (mouseY + scrollAmount()) >= b.getY()
                && (mouseY + scrollAmount()) < b.getY() + b.getHeight();
    ...
    protected void renderBorder(GuiGraphics guiGraphics, int x, int y, int width, int height) {
        guiGraphics.fill(x, y, x + width, y + height, EBDClient.colorData.getBackgroundColor());
    ...
    protected void renderScrollBar(GuiGraphics guiGraphics) {
        ...
        int maxY = minY + i;
        guiGraphics.fill(minX, minY, maxX, maxY, EBDClient.colorData.getSliderOuterColor());
        guiGraphics.fill(minX + 1, minY + 1, maxX - 1, maxY -1, EBDClient.colorData.getSliderInnerColor()); //-8241880 . -11789813 //og -8355712 . -4144960 //purple -4814674 . -345617
    }
